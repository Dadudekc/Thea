...
class DiscordManager:
        ...
        try:
            ...
            for env_key, logical_key in _channel_env_map.items():
                ...
                if env_val:
                    ...
                    logger.info(f"[OK] {logical_key.capitalize()} channel loaded from {env_key}")
            # EDIT END
    ...
    def _register_commands(self):
        ...
        @self.bot.tree.command(name="dreamscape", description="Get current dreamscape status")
        async def dreamscape_status(interaction):
            ...
            try:
                ...
                await interaction.response.send_message(f"❌ Error getting dreamscape status: {e}")
        
        @self.bot.tree.command(name="quests", description="Quest commands: list / accept / complete")
        async def quests_command(interaction, action: str = "list", quest_id: str = None):
            try:
                if not self.mmorpg_engine:
                    await interaction.response.send_message("❌ MMORPG engine not available", ephemeral=True)
                    return
                ...
                if action == "list":
                    ...
                    if available:
                        embed.add_field(name="🆕 Available", value="\n".join(f"`{q.id}` {q.title}" for q in available[:10]), inline=False)
                    else:
                        embed.add_field(name="🆕 Available", value="None", inline=False)
                    if active:
                        embed.add_field(name="🟡 Active", value="\n".join(f"`{q.id}` {q.title}" for q in active[:10]), inline=False)
                    await interaction.response.send_message(embed=embed)

                elif action == "accept":
                    if not quest_id:
                        await interaction.response.send_message("❌ Provide quest_id: `/quests accept quest_id:<id>`", ephemeral=True)
                        return
                    ok = self.mmorpg_engine.accept_quest(quest_id)
                    if ok:
                        await interaction.response.send_message(f"🟡 Quest `{quest_id}` accepted!")
                    else:
                        await interaction.response.send_message("❌ Quest not found or already active.", ephemeral=True)

                elif action == "complete":
                    if not quest_id:
                        await interaction.response.send_message("❌ Provide quest_id: `/quests complete quest_id:<id>`", ephemeral=True)
                        return
                    ok = self.mmorpg_engine.complete_quest(quest_id)
                    if ok:
                        await interaction.response.send_message(f"✅ Quest `{quest_id}` completed! XP awarded.")
                    else:
                        await interaction.response.send_message("❌ Quest not found or not active.", ephemeral=True)

                else:
                    await interaction.response.send_message("❌ Invalid action. Use `list`, `accept`, or `complete`", ephemeral=True)

            except Exception as e:
                await interaction.response.send_message(f"❌ Error with quests command: {e}")
        
        @self.bot.tree.command(name="skills", description="Show player skills and progression")
        async def show_skills(interaction):
            """Show player skills and progression."""
            try:
                if not self.mmorpg_engine:
                    await interaction.response.send_message("❌ MMORPG engine not available")
                    return
                ...
                skill_list = "\n".join([f"• {skill.name} (Lv {skill.level})" for skill in skills[:5]])
                embed.add_field(
                    name="Active Skills",
        ...
        @self.bot.tree.command(name="domains", description="Show empire domains and territories")
        async def show_domains(interaction):
            ...
            try:
                ...
                if domains:
                    domain_list = "\n".join([f"• {domain.name} ({domain.type})" for domain in domains[:5]])
                    embed.add_field(name="Active Domains", value=domain_list, inline=False)
                else:
                    embed.add_field(name="Domains", value="No domains conquered yet", inline=False)
                
                await interaction.response.send_message(embed=embed)
                
            except Exception as e:
                await interaction.response.send_message(f"❌ Error getting domains: {e}")
        
        @self.bot.tree.command(name="process", description="Manually trigger conversation processing")
        async def manual_process(interaction):
            """Manually trigger conversation processing."""
            try:
                # Send initial response
                await interaction.response.send_message("🔄 Starting manual processing...")
                ...
                try:
                    ...
                    if not live_proc:
                        await interaction.followup.send("❌ Live processor not available")
                        return
                    
                    # Check current status
                    current_status = live_proc.get_status().status.value
                    if current_status == "processing":
                        await interaction.followup.send("⏳ Processing already in progress. Please wait for current process to complete.")
                        return
                    
                    # Trigger processing
                    await interaction.followup.send("⚡ Triggering conversation processing...")
                    
                    # Start processing in background
                    asyncio.create_task(self.run_processing_with_updates(interaction, live_proc))
                    
                except ImportError:
                    await interaction.followup.send("❌ Live processor module not available")
                except Exception as e:
                    await interaction.followup.send(f"❌ Error triggering processing: {e}")
                
            except Exception as e:
                await interaction.response.send_message(f"❌ Error with process command: {e}")
        
        @self.bot.tree.command(name="stats", description="Show detailed processing statistics")
        async def show_stats(interaction):
            ...
            try:
                ...
                if live_proc:
                    ...
                    embed.add_field(name="Status", value="Live processor not available", inline=False)
        ...
        @self.bot.tree.command(name="help", description="Show available commands and help")
        ...
        async def guild_command(interaction, action: str = "info", name: str = None, description: str = None):
            ...
            try:
                ...
                if action == "create":
                    if not name:
                        await interaction.response.send_message("❌ Usage: `/guild create name:<GuildName> description:<optional>`", ephemeral=True)
                        return
                    ok = self.mmorpg_engine.create_guild(name, description or "", str(interaction.user.id))
                    if ok:
                        await interaction.response.send_message(f"🏰 Guild **{name}** created! {interaction.user.mention} is the leader.")
                    else:
                        await interaction.response.send_message(f"❌ Guild **{name}** already exists.", ephemeral=True)

                elif action == "join":
                    if not name:
                        await interaction.response.send_message("❌ Usage: `/guild join name:<GuildName>`", ephemeral=True)
                        return
                    ok = self.mmorpg_engine.join_guild(name, str(interaction.user.id))
                    if ok:
                        await interaction.response.send_message(f"✅ {interaction.user.mention} joined **{name}**!")
                    else:
                        await interaction.response.send_message(f"❌ Unable to join guild **{name}** – does it exist, or are you already a member?", ephemeral=True)

                elif action == "info":
                    if not name:
                        await interaction.response.send_message("❌ Usage: `/guild info name:<GuildName>`", ephemeral=True)
                        return
                    info = self.mmorpg_engine.get_guild_info(name)
                    if not info:
                        await interaction.response.send_message(f"❌ Guild **{name}** not found", ephemeral=True)
                        return
                    embed = discord.Embed(title=f"🏰 Guild: {info['name']}", description=info['description'] or 'No description', color=0x3498db)
                    embed.add_field(name="Leader", value=f"<@{info['leader']}>", inline=True)
                    embed.add_field(name="Members", value=str(info['member_count']), inline=True)
                    if info['members']:
                        embed.add_field(name="Member List", value="\n".join(f"<@{m}>" for m in info['members'][:20]), inline=False)
                    await interaction.response.send_message(embed=embed)
                    
                else:
                    await interaction.response.send_message("❌ Invalid action. Use: `create`, `join`, or `info`")
                    
            except Exception as e:
                await interaction.response.send_message(f"❌ Error with guild command: {e}")
        
        @self.bot.tree.command(name="trade", description="Trading system commands")
        async def trade_command(interaction, action: str = "list", target: str = None, item: str = None, amount: int = 1):
            """Trading system commands."""
            try:
                if action == "offer":
                    if not target or not item:
                        await interaction.response.send_message("❌ Please provide target and item: `/trade offer target:@user item:ItemName amount:1`")
                        return
                    ...
                    embed.add_field(name="Status", value="🔄 Coming Soon", inline=True)
                    embed.add_field(name="Features", value="Trade offers, market prices, history", inline=True)
                    
                    await interaction.response.send_message(embed=embed)
                    
                else:
                    await interaction.response.send_message("❌ Invalid action. Use: `offer`, `accept`, or `list`")
                    
            except Exception as e:
                await interaction.response.send_message(f"❌ Error with trade command: {e}")
        
        @self.bot.tree.command(name="market", description="Market and economy information")
        async def market_command(interaction, item: str = None):
            ...
            try:
                ...
                await interaction.response.send_message(f"❌ Error getting market data: {e}")
        
        @self.bot.tree.command(name="profile", description="Show detailed player profile")
        async def profile_command(interaction):
            ...
            try:
                ...
                try:
                    ...
                    embed.add_field(name="🔄 Processing", value="Stats unavailable", inline=True)
                
                embed.set_thumbnail(url="https://via.placeholder.com/64x64/9b59b6/ffffff?text=DO")
                await interaction.response.send_message(embed=embed)
                
            except Exception as e:
                await interaction.response.send_message(f"❌ Error getting profile: {e}")
        
        @self.bot.tree.command(name="leaderboard", description="Show player leaderboards")
        async def leaderboard_command(interaction, category: str = "xp"):
            ...
            try:
                ...
                await interaction.response.send_message(f"❌ Error getting leaderboard: {e}")
        
        @self.bot.tree.command(name="convo", description="Conversation utilities (summary, info)")
        async def convo_command(interaction, action: str = "summary", conversation_id: str = None):
            """Provide conversation summaries and meta information stored in the DB.

            Usage:
              /convo summary <conversation_id>
            """
            from core.memory_manager import MemoryManager  # Lazy import to avoid circular deps

            if action not in ["summary", "info"]:
                await interaction.response.send_message("Invalid action. Use 'summary' or 'info'", ephemeral=True)
                return

            if not conversation_id:
                await interaction.response.send_message("Please supply a conversation_id", ephemeral=True)
                return
            ...
            try:
                ...
                if not convo:
                    await interaction.response.send_message(f"Conversation '{conversation_id}' not found.", ephemeral=True)
                    return

                if action == "summary":
                    summary = convo.get("summary") or "No summary stored yet – run analyze_conversations_ai.py first."
                ...
                await interaction.response.send_message("Error fetching conversation.", ephemeral=True)
    ...
    async def send_dreamscape_update(self, message: str, embed_data: Optional[Dict] = None):
    ...
    async def run_processing_with_updates(self, interaction, live_proc):
        ...
        try:
            ...
            processed_diff = updated_stats.total_conversations_processed - initial_stats.total_conversations_processed
    ...
    def get_channel(self, key: str):  # type: ignore[return-value]
        ...
        fallback_cid = self.config.get("channel_id") or self.config.get("channels", {}).get("default")
        return self.bot.get_channel(int(fallback_cid)) if fallback_cid else None

    async def send_update(self, key: str, message: str, embed_data: Optional[Dict] = None):
...
def main():
    ...
    print(f"Environment Token: {'Available' if os.getenv('DISCORD_BOT_TOKEN') else 'Not Set'}")
    print(f"Environment App ID: {'Available' if os.getenv('DISCORD_APPLICATION_ID') else 'Not Set'}")
